{if $option->issortable}
    <script data-relocate="true">
        document.addEventListener('DOMContentLoaded', function() {
            require(['Dom/Traverse', 'Dom/Util', 'Ui/TabMenu', 'WoltLabSuite/Core/Ui/Sortable/List'], function(
                DomTraverse, DomUtil, UiTabMenu, UiSortableList) {
                var sortableList = elById('{$option->optionName}SortableList');
                var tabMenu = UiTabMenu.getTabMenu(DomUtil.identify(DomTraverse.parentByClass(sortableList,
                    'tabMenuContainer')));
            ...
            <span class="sortableNodeLabel">
                <span class="sortableNodeHandle">
                    {icon name='arrows'}
                        ...
                        <label>
                            <input type="checkbox" name="values[{$option->optionName}][]" value="{$availableDatabase}"
                                {if $availableDatabase|in_array:$value} checked{/if}>
